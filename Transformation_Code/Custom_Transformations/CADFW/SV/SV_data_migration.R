...
data_path <- "/Users/anthonyngo/Documents/Wildlife_Insights/CDFW/deerimagefiles/SV/SV_2071_ImageData.xlsx"

# Path of data in BU formatting
cam_path <- "/Users/anthonyngo/Documents/Wildlife_Insights/CDFW/deerimagefiles/SV/camera.csv"
dep_path <- "/Users/anthonyngo/Documents/Wildlife_Insights/CDFW/deerimagefiles/SV/deployment.csv"
prj_path <- "/Users/anthonyngo/Documents/Wildlife_Insights/CDFW/deerimagefiles/SV/project.csv"
...
data <- read_excel(data_path, sheet=3)

data$year <- "2017"
data$Reviewer <- "Maring, Bullington"

## Loading in the Event-Species mappings\
key <- read_excel(data_path, sheet=4)
...
wi_key[nrow(wi_key) + 1,] = list("UNID",NA,NA,NA,NA,NA,NA,"1f689929-883d-4dae-958c-3d57ab5b6c16")
wi_key[nrow(wi_key) + 1,] = list("VEHICLE",NA,NA,NA,NA,NA,NA,"1a6d048a-28da-45bb-845d-a14cc5b7d61a")
wi_key[nrow(wi_key) + 1,] = list("UNBIRD",NA,NA,NA,NA,NA,NA,"b1352069-a39c-4a84-a949-60044271c0c1")
wi_key[nrow(wi_key) + 1,] = list("CATTLE",NA,NA,NA,NA,NA,NA,"aca65aaa-8c6d-4b69-94de-842b08b13bd6")
wi_key[nrow(wi_key) + 1,] = list("DOMCAT",NA,NA,NA,NA,NA,NA,"9212982e-8a58-4775-a6ac-e9a43110d8f5")
wi_key[nrow(wi_key) + 1,] = list("HORSE",NA,NA,NA,NA,NA,NA,"5109acb4-e503-4147-a175-a3c6aa71f1e3")
wi_key[nrow(wi_key) + 1,] = list("UNCANID",NA,NA,NA,NA,NA,NA,"3184697f-51ad-4608-9a28-9edb5500159c")
wi_key[nrow(wi_key) + 1,] = list("DOMSHE",NA,NA,NA,NA,NA,NA,"0de8422e-f59d-4802-9e93-ab8559e43e55")
wi_key[nrow(wi_key) + 1,] = list("BKBMAG",NA,NA,NA,NA,NA,NA,"f6360527-6dfb-4cfa-890d-a522868ce416")
wi_key[nrow(wi_key) + 1,] = list("WESJAY",NA,NA,NA,NA,NA,NA,"f76dea54-8d1d-4483-846b-a5c47ea7d4eb")
wi_key[nrow(wi_key) + 1,] = list("STEJAY",NA,NA,NA,NA,NA,NA,"68b01aa1-dfc1-46a6-bcac-87767647ad17")
wi_key[nrow(wi_key) + 1,] = list("CALQUA",NA,NA,NA,NA,NA,NA,"daf972d0-b038-4d10-8862-01d0e590e551")
wi_key[nrow(wi_key) + 1,] = list("MOUQUA",NA,NA,NA,NA,NA,NA,"5ff8c8fd-372e-4bfb-8ca2-18e33780c797")
wi_key[nrow(wi_key) + 1,] = list("AMEROB",NA,NA,NA,NA,NA,NA,"23140944-2bd7-46ce-819a-c26508c53d62")
wi_key[nrow(wi_key) + 1,] = list("UNBUTTERFLY",NA,NA,NA,NA,NA,NA,"4a7126ec-2b0b-426d-bafd-356219eb487a")
wi_key[nrow(wi_key) + 1,] = list("CANLAT",NA,NA,NA,NA,NA,NA,"aaf3b049-36e6-46dd-9a07-8a580e9618b7")
wi_key[nrow(wi_key) + 1,] = list("SPEBEE",NA,NA,NA,NA,NA,NA,"d01f67fd-836b-4b25-89fc-2239e59f56b0")
wi_key[nrow(wi_key) + 1,] = list("DOMDOG",NA,NA,NA,NA,NA,NA,"3d80f1d6-b1df-4966-9ff4-94053c7a902a")
wi_key[nrow(wi_key) + 1,] = list("DIDVIR",NA,NA,NA,NA,NA,NA,"87be3a5c-e60a-4e7e-88c7-21544914d067")
wi_key[nrow(wi_key) + 1,] = list("UNDIPOD",NA,NA,NA,NA,NA,NA,"583c7fae-59f0-40f1-9ce7-484717f2f0ad")
wi_key[nrow(wi_key) + 1,] = list("UNSYVIL",NA,NA,NA,NA,NA,NA,"cacc63d7-b949-4731-abce-a403ba76ee34")
wi_key[nrow(wi_key) + 1,] = list("GRHOWL",NA,NA,NA,NA,NA,NA,"b9f145ef-bd00-4d66-93ba-6e3fb4e51e9f")
wi_key[nrow(wi_key) + 1,] = list("RINPHE",NA,NA,NA,NA,NA,NA,"9ba3565d-9934-4e74-8ef4-d110ad587014")
wi_key[nrow(wi_key) + 1,] = list("BREBLA",NA,NA,NA,NA,NA,NA,"0364fc81-17d7-4495-a48c-4fc396b44d5d")
wi_key[nrow(wi_key) + 1,] = list("UNREPTILE",NA,NA,NA,NA,NA,NA,"739a105e-d883-4ff8-9282-7ec44018e6a0")
wi_key[nrow(wi_key) + 1,] = list("MOURDOV",NA,NA,NA,NA,NA,NA,"0ccfe789-41e3-45b8-bdbc-36f0ce73db48")
wi_key[nrow(wi_key) + 1,] = list("WHCSPA",NA,NA,NA,NA,NA,NA,"fd02a22d-cff0-40e3-98f5-8a7f77382463")
wi_key[nrow(wi_key) + 1,] = list("MOUDOV",NA,NA,NA,NA,NA,NA,"0ccfe789-41e3-45b8-bdbc-36f0ce73db48")
wi_key[nrow(wi_key) + 1,] = list("NORFLI",NA,NA,NA,NA,NA,NA,"02799ea2-fba0-4883-b27e-b41ae387e884")
wi_key[nrow(wi_key) + 1,] = list("UA",NA,NA,NA,NA,NA,NA,"627c919c-29bc-439b-acde-62b3cd31f8a8")
wi_key[nrow(wi_key) + 1,] = list("AMECRO",NA,NA,NA,NA,NA,NA,"87fdd451-ca1e-47b1-af43-aa4517cd13bf")
wi_key[nrow(wi_key) + 1,] = list("AMEKES",NA,NA,NA,NA,NA,NA,"1ab198a0-04dc-4407-8370-1a458c0c9f8e")
wi_key[nrow(wi_key) + 1,] = list("GRBHER",NA,NA,NA,NA,NA,NA,"96fe1a07-7ef1-4a2f-99e1-ec2c9a78b532")
wi_key[nrow(wi_key) + 1,] = list("GREROA",NA,NA,NA,NA,NA,NA,"2c5de670-22bf-4352-87b9-8ece74442e85")
wi_key[nrow(wi_key) + 1,] = list("YEBMA",NA,NA,NA,NA,NA,NA,"c1750080-1f8e-4238-b121-066d700f1641")
wi_key[nrow(wi_key) + 1,] = list("EURSTA",NA,NA,NA,NA,NA,NA,"8a50bd14-97f4-4f76-8e6c-5edeeb5be43f")
wi_key[nrow(wi_key) + 1,] = list("INDPEA",NA,NA,NA,NA,NA,NA,"f90f90b5-b363-489d-ac71-feea6c780f56")
wi_key[nrow(wi_key) + 1,] = list("CALTHR",NA,NA,NA,NA,NA,NA,"c390a069-ea08-415a-9a86-ff485090b787")
wi_key[nrow(wi_key) + 1,] = list("RETHAW",NA,NA,NA,NA,NA,NA,"c0206b78-8e3a-4612-84c7-ba13a36ba3f2")
wi_key[nrow(wi_key) + 1,] = list("DIPMIC",NA,NA,NA,NA,NA,NA,"59f9ef45-252b-4b18-a159-954a5e25ef19")
wi_key[nrow(wi_key) + 1,] = list("SLYNUT",NA,NA,NA,NA,NA,NA,"3a292fc0-f364-4c55-bf4b-454cdb249117")
wi_key[nrow(wi_key) + 1,] = list("OLSFLY",NA,NA,NA,NA,NA,NA,"3c6421e2-58b6-4d5f-b615-ca207b042067")
wi_key[nrow(wi_key) + 1,] = list("CHBCHI",NA,NA,NA,NA,NA,NA,"be1f040c-93e8-487f-bbe7-1276d2e96422")
wi_key[nrow(wi_key) + 1,] = list("NEOSEN",NA,NA,NA,NA,NA,NA,"3c727840-a266-492f-b403-d3af7a595121")
...
img_raw$Reference <- sub("-$", "", img_raw$Reference)
img_raw$Reference <- sub("TO", "-", img_raw$Reference)
...
img_raw[is.na(img_raw$Site),"Station"] <- "L"
img_raw[is.na(img_raw$Site),"Site"] <- "SV7"

img_raw$cam <- "NONE"
img_raw[img_raw$Site=="SV1" & img_raw$Station=="K", "cam"] <- "102RECNX"
img_raw[img_raw$Site=="SV1" & img_raw$Station=="L", "cam"] <- "100RECNX"

img_raw[img_raw$Site=="SV2" & img_raw$Station=="L", "cam"] <- "100RECNX"

img_raw[img_raw$Site=="SV3" & img_raw$Station=="L", "cam"] <- "100RECNX"

img_raw[img_raw$Site=="SV5" & img_raw$Station=="K", "cam"] <- "100RECNX"
img_raw[img_raw$Site=="SV5" & img_raw$Station=="L", "cam"] <- "100RECNX"

img_raw[img_raw$Site=="SV6" & img_raw$Station=="K", "cam"] <- "100RECNX"
img_raw[img_raw$Site=="SV6" & img_raw$Station=="L", "cam"] <- "100RECNX"

img_raw[img_raw$Site=="SV7" & img_raw$Station=="K", "cam"] <- "101RECNX"
img_raw[img_raw$Site=="SV7" & img_raw$Station=="L", "cam"] <- "100RECNX"

img_raw$location <- paste("gs://cadfw/Deer_Program",
                          prj_id,
                          img_raw$Site,img_raw$Station, img_raw$cam,
...
img_raw[img_raw$dep_id=="SV5L-2017", "dep_id"] <- "SV5 L-2017"
# Mapping WI Taxon values
img_raw <- left_join(img_raw, wi_key, by="Event", all.x = TRUE)

# Adding Timestamps----
timestamps <- read.csv("~/Documents/Wildlife_Insights/CDFW/deerimagefiles/SV/SV_exifs.csv")
...
# img_bu <- img_bu[img_bu$location %in% trueLocations$locations,]
# rm(img_raw, indices, unlisted_ref, img_count, timestamps)
write.table(cam_bu,file="/Users/anthonyngo/Documents/Wildlife_Insights/CDFW/deerimagefiles/SV/output/cameras.csv", row.names = FALSE, sep=",")
write.table(dep_bu,file="/Users/anthonyngo/Documents/Wildlife_Insights/CDFW/deerimagefiles/SV/output/deployments.csv", row.names = FALSE, sep=",")
write.table(img_bu,file="/Users/anthonyngo/Documents/Wildlife_Insights/CDFW/deerimagefiles/SV/output/images.csv", row.names = FALSE, sep=",")
write.table(prj_bu,file="/Users/anthonyngo/Documents/Wildlife_Insights/CDFW/deerimagefiles/SV/output/projects.csv", row.names = FALSE, sep=",")
